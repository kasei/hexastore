	...
	if(safe_malloc((void**)&ses, sizeof(async_des_session)) > 0) {
		...
		if(fixed_size > 0) {
			for(i = 0; i < num_sends; i++) {
				...
				ses->sends[i] = async_mpi_session_create3(buf, fixed_size, -1, ASYNC_MPI_FLAG_IS_SENDING | ASYNC_MPI_FLAG_FREE_BUF | ASYNC_MPI_FLAG_FREE_REQUEST);
			...
			for(i = 0; i < num_sends; i++) {
				ses->sends[i] = async_mpi_session_create3(NULL, -1, -1, ASYNC_MPI_FLAG_IS_SENDING | ASYNC_MPI_FLAG_SHOULD_SEND_COUNT | ASYNC_MPI_FLAG_FREE_BUF | ASYNC_MPI_FLAG_FREE_REQUEST);
			}
			for(i = 0; i < num_recvs; i++) {
...
//#define _CHECK_R(r)
//#define ASYNC_DEBUG(...) fprintf(stdout, __VA_ARGS__)
#define ASYNC_DEBUG(...)
enum async_status async_des(void* session) {
	ASYNC_DEBUG("async_des\n");
