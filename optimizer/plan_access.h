...
typedef enum {
	HX_OPTIMIZER_ACCESS_PLAN_INDEX	= 0,	// access to triple pattern matching via hx_index
	HX_OPTIMIZER_ACCESS_PLAN_LAST	= HX_OPTIMIZER_ACCESS_PLAN_INDEX
} hx_optimizer_plan_access_type;

typedef struct {
	hx_optimizer_plan_access_type type;
	void* source;
	hx_triple* triple;
	int order_count;
	hx_variablebindings_iter_sorting** order;
} hx_optimizer_access_plan;
