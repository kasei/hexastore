...
void help (int argc, char** argv) {
	fprintf( stderr, "Usage: %s data.rdf nodemap.tcb triples.dat\n\n", argv[0] );
}
...
hx_node* parser_node ( parser_t* parser, void* node, raptor_identifier_type type, char* lang, raptor_uri* dt ) {
	...
	if (tchdbputkeep(parser->cabinet, string, strlen(string), &(parser->next_id), sizeof(uint64_t))) {
		parser->next_id++;
...
int main (int argc, char** argv) {
	...
	parser->cabinet	= tchdbnew();
	if(!tchdbopen(parser->cabinet, nodemap_filename, HDBOWRITER | HDBOCREAT)) {
		ecode = tchdbecode(parser->cabinet);
	...
	void* p;
	int len;
	if (p = tchdbget(parser->cabinet, "   NEXTID", 9, &len)) {
	...
	logger( parser->count );
	fprintf( stderr, "\n" );
	tchdbput(parser->cabinet, "   NEXTID", 9, &( parser->next_id ), sizeof( uint64_t ));
	
	tchdbiterinit(parser->cabinet);
	while((p = tchdbiternext(parser->cabinet, &len)) != NULL) {
		uint64_t* idp;
		int idlen;
		idp	= tchdbget(parser->cabinet, p, len, &idlen);
		if (idp) {
			printf("%10"PRIu64"\t%s\n", *idp, p);
			free(idp);
		}
